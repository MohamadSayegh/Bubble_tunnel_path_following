...
Questions:
    ...
    units of W ==== constraints ?

    s could be 1 other than final point == but in obstacles s does not start/ end at limit points
...
# ---------------- eliminates the need of end point constraints !?
...
# not helping the solution. maybe wrong ?

# v_guess = np.ones(N)
# for i in range(0,N-1):
#     vx = (global_path_guess_x[i+1] - global_path_guess_x[i])/dt 
#     vy = (global_path_guess_y[i+1] - global_path_guess_y[i])/dt 
#     v_guess[i+1] = np.sqrt(vx**2 + vy**2)
    
v_guess = 0.5*np.ones(N)
ocp.set_initial(v , v_guess)

w_guess = 0.5*np.ones(N)
...
# ocp.add_objective(-1e3*ocp.at_tf(s1))

ocp.add_objective(1e-6*ocp.integral(v**2 + w**2))


# ocp.add_objective(ocp.integral( sumsqr( (x - global_path_guess_x) + (y - global_path_guess_y))))
