...
while len(shifted_midpoints_x) < N:
    shifted_midpoints_x.append(global_path_x[-1])
    shifted_midpoints_y.append(global_path_y[-1])
...
ocp.set_initial(v , 0.5*np.ones(N))
ocp.set_initial(w , 0.5*np.ones(N))
...
# ocp.add_objective( 1*ocp.sum(s_obs))
...
for i in range(Nsim):
    ...
    ocp.set_value( end_goal_x, shifted_midpoints_x[-1])
    ocp.set_value( end_goal_y, shifted_midpoints_y[-1])
