...
#shooting points 
N = 100
dt = 0.1 # has to be low for good path following 
...
# ocp.add_objective(3*( slack_tf_x_2  + slack_tf_y_2   ))
# ocp.add_objective(3*( slack_tf_x    + slack_tf_y     ))


ocp.add_objective(10*ocp.integral(( x - pf[0])**2 + (y-pf[1])**2 )) 


#-------------- Add path 

path_x = np.linspace(0, 5, 5)
path_y = [  0.    ,  1.0   , 2.0   , 3.0   , 5.0]
...
ocp.add_objective(2*ocp.integral((x - path_spline_x(s1))**2 + (y-path_spline_y(s1))**2)) 
