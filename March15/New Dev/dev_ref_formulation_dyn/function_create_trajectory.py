...
def get_occupied_positions(psx, psy, rs):
    ...
    for i in range(0, len(rs)):
        ...
        circle_x = psx[i]
        circle_y = psy[i]
        occupied_positions_x = np.concatenate((occupied_positions_x, circle_x + radius*np.cos(ts) ))
        occupied_positions_y = np.concatenate((occupied_positions_y, circle_y + radius*np.sin(ts) ))
