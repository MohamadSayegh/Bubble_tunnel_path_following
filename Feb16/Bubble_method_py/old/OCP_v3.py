...
fixes: 
    ...
    3) remove the condition of staying within the next bubble
    
    4) making the staying within bubble as hard constraint => no slack varaiables
...
ocp.set_initial(s1, np.linspace(tunnel_s1[0],tunnel_s1[-3], N)) 
...
ocp.set_initial(s2, np.linspace(tunnel_s2[0],tunnel_s2[-3], N)) 
ocp.set_initial(sdot2, (tunnel_s2[-2]-tunnel_s2[0])/tlength2)
ocp.subject_to(ocp.at_tf(s2) < 1)

path_spline_x = interpolant('x','bspline',[tunnel_s2],path_x,{"algorithm": "smooth_linear","smooth_linear_frac":0.49})  #should use low fraction ?
...
# global_path_param_x = np.array(global_path[0])
# global_path_param_y = np.array(global_path[1])


# ocp.set_initial(x,      xsol)
# ocp.set_initial(y,      ysol)
...
tolerance = 3  #adding this tolerance has reduced solution time and also gave better solution, but at someplaces it should 
